...
class Transform {
  ...
  Transform(const Mat4& _mat, const Mat4& _invmat) : mat(_mat), invmat(_invmat) {};
  ...
  Vec3 applyPoint(const Vec3& v) const {
    const Real x = mat.m[0][0] * v.x + mat.m[0][1] * v.y + mat.m[0][2] * v.z + mat.m[0][3];
    const Real y = mat.m[1][0] * v.x + mat.m[1][1] * v.y + mat.m[1][2] * v.z + mat.m[1][3];
    const Real z = mat.m[2][0] * v.x + mat.m[2][1] * v.y + mat.m[2][2] * v.z + mat.m[2][3];
    const Real w = mat.m[3][0] * v.x + mat.m[3][1] * v.y + mat.m[3][2] * v.z + mat.m[3][3];

    if (w == 1) return Vec3(x, y, z);
    else return Vec3(x/w, y/w, z/w);
  };
  ...
  Vec3 applyNormal(const Vec3& n) const {
    return Vec3(
      ...
      invmat.m[0][2] * n.x + invmat.m[1][1] * n.y + invmat.m[2][2] * n.z
    );
  };
};
